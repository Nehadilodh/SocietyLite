...
const ComplaintDetail = () => {
    ...
    useEffect(() => {
        const fetchComplaint = async () => {
            try {
                // Since there is no single GET /complaint/:id without admin fetching from all,
                // we'll fetch all and filter or assume there's a way.
                ...
                <div className="flex justify-between items-start mb-6">
                    <div>
                        ...
                        <p className={`text-sm ${darkMode ? 'text-slate-400' : 'text-slate-500'}`}>
                            By {complaint.raisedBy?.name} (Flat {complaint.raisedBy?.flatNo}) on {new Date(complaint.createdAt).toLocaleDateString()}
                        </p>
                    </div>
